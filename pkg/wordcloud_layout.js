/* @ts-self-types="./wordcloud_layout.d.ts" */
 ...
 * @returns {any}
 */
export function layout_treemap(input, config) {
    const ret = wasm.layout_treemap(input, config);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function layout_words(input, mode, config) {
    const ret = wasm.layout_words(input, mode, config);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./wordcloud_layout_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('wordcloud_layout_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
